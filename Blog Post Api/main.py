...
def create_post(posts: Posts):
	...
	values = (posts.name, posts.timestamp, posts.post)

	try:
		cursor.execute(sql, values)
		conn.commit()
		return json.dumps({'Message': 'Succesfully created your post'})
	except Exception as e:
		return json.dumps({f'Error': 'Something went wrong {str{e}}'})
